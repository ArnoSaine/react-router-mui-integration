...
import isErrnoException from "../isErrnoException.ts";
import cleanOldFiles from "./cleanOldFiles.ts";
import touchFile from "./touchFile.ts";
...
export default async function memoize(
  ...
  try {
    ...
    //console.log(`Cache miss. Running: ${name}`);
